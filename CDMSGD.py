...
def model_compilers_cdmsgd(model,n_agents,optimizer,pi,opt):
    global parameters
    ...
    for nb in range(n_agents):
        ...
        opt[nb] = CDMSGD(lr=1E-2, decay=0, momentum=0.95, nesterov=True, optparam=optparam[nb])
    # Compile model
    for nb in range(n_agents):
...
def update_parameters_cdmsgd(agentmodels,n_agents):
    global parameters
...
class CDMSGD(Optimizer):
    ...
    def get_config(self):
        ...
        base_config = super(CDMSGD, self).get_config()
